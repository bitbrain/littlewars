...
public class SpawnArea {
	 ...
	 */
	private void queryRadiusRange(int tileX, int tileY, int range, CommandoCenter center) {
		queryRadiusRange(tileX, tileY, range, center, false);
	}
	private void queryRadiusRange(int tileX, int tileY, int range, CommandoCenter center, boolean deleting) {
		if (range > 0) {			
			int x0 = center.getTileX();
			int y0 = center.getTileY();
		...
		if (range > 0) {
			queryRadiusRange(tileX, tileY, --range, center, deleting);
		}
	}
}
